...
contract Vault is Ownable, ReentrancyGuard {
    ...
    function addVaultToken(address _tokenAddress, uint _minLockAmount) external onlyOwner {
        require(_tokenAddress != address(0), "Invalid token address");
        require(
            vaultTokens[_tokenAddress].tokenAddress == address(0),
            "Token already added"
        );

        vaultTokens[_tokenAddress] = VaultToken({
            tokenAddress: _tokenAddress,
            conversionRate: 0,
            totalLockedTokens: 0,
        ...
        vaultTokenAddresses.push(_tokenAddress);
    }

    function setVaultTokenConversionRate(
        ...
        if (_nEpochs > 0) {
            vaultTokens[_tokenAddress].nEpochs += _nEpochs;
        }
        if (_conversionRate > 0) {
            vaultTokens[_tokenAddress].conversionRate = _conversionRate;
        }
    }

    /// @notice Function to remove a reward token
    /// @param _tokenAddress Address of the reward token
    ...
    function removeVaultToken(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(0), "Invalid token address");
        require(
            vaultTokens[_tokenAddress].tokenAddress != address(0),
            "Token not added"
        ...
        delete vaultTokens[_tokenAddress];
        for (uint i = 0; i < vaultTokenAddresses.length; i++) {
            if (vaultTokenAddresses[i] == _tokenAddress) {
                vaultTokenAddresses[i] = vaultTokenAddresses[
                    vaultTokenAddresses.length - 1
                ];
                vaultTokenAddresses.pop();
                break;
            }
        }
    }

    /// @notice Function to set the max number of active users
    /// @param _maxActiveUsers Max number of active users
    function setMaxActiveUsers(uint _maxActiveUsers) external onlyOwner {
        require(_maxActiveUsers > 0, "Invalid max active users");
        maxActiveUsers = _maxActiveUsers;
    }
    ...
    function lockTokens(uint _amount) external nonReentrant {
        ...
        // Manage active users
        if (userLockInfo[msg.sender].lockedTokensAddresses.length == 0) {
            activeUsers.push(msg.sender);
        }

        userLockInfo[msg.sender].lockedTokens = netAmount;
        userLockInfo[msg.sender].virtualLockedTokens = netAmount;
        userLockInfo[msg.sender].lockStartBlock = block.number;
        userLockInfo[msg.sender].lockEndBlock = block.number + LOCK_PERIOD;
    ...
    function lockTokensPartner(
        uint _amount,
        address _vaultToken
    ) external nonReentrant {
        require(_amount >= vaultTokens[_vaultToken].minLockAmount, "Amount below minimum requirement");
        IERC20 token = IERC20(_vaultToken);
        require(
            activeUsers.length <= maxActiveUsers,
            "Max users limit reached"
        );
        ...
        if (userLockInfo[msg.sender].lockedTokensAddresses.length != 0) {
            for (uint i = 0; i < userLockInfo[msg.sender].lockedTokensAddresses.length; i++) {
                require(userLockInfo[msg.sender].lockedTokensAddresses[i] != _vaultToken, "Token already locked");
            }
        } else if (userLockInfo[msg.sender].lockedTokens == 0) {
            activeUsers.push(msg.sender);
        }
        require(
        ...
        // Transfer the fee and the net amount
        token.safeTransferFrom(msg.sender, feeBeneficiary, feeAmount);
        token.safeTransferFrom(msg.sender, address(this), netAmount);

        userLockInfo[msg.sender].lockedTokensAddresses.push(_vaultToken);
        userLockInfo[msg.sender].lockedTokensAmounts.push(netAmount);
        userLockInfo[msg.sender].lockedTokensStartBlocks.push(block.number);
        userLockInfo[msg.sender].lockedTokensEndBlocks.push(block.number + LOCK_PERIOD);
